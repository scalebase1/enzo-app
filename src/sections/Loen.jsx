...
import { Segmentvaelger, Noegletalskort } from '../komponenter/index.jsx'
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 16px', borderTop: i > 0 ? `1px solid ${c.line}` : 'none', opacity: daempet ? 0.55 : 1, flexWrap: 'wrap' }}>
      <div style={{ flex: 1, minWidth: 130 }}>
        <div style={{ fontSize: 14, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.navn}</div>
      ...
      <div style={{ fontSize: 15, fontWeight: 500, minWidth: 96, textAlign: 'right', color: daempet ? c.slate2 : c.ink }}>{kr(m.loen)}</div>
    </div>
  )
}
...
export default function Loen() {
  ...
  return (
    <div style={{ fontFamily: font }}>
      ...
      <Segmentvaelger muligheder={PERIODER} valgt={periode} onVaelg={setPeriode} style={{ margin: '16px 0' }} />
      ...
      {!loading && !err && data && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: sp(3) }}>
            <Noegletalskort label="Timer i alt" vaerdi={timerFmt(total.timer)} />
            <Noegletalskort label="Løn i alt" vaerdi={kr(total.loen)} />
            <Noegletalskort label="Medarbejdere med timer" vaerdi={Number(total.medarbejdere_med_timer || 0).toLocaleString('da-DK')} />
          </div>
          ...
          <div style={{ ...card, padding: 0, overflow: 'hidden', marginTop: sp(3) }}>
            {medarbejdere.length === 0 ? (
              ...
              <>
                ...
                {udenTimer.length > 0 && (
                  <>
                    <div style={{ padding: '10px 16px', fontSize: 11, fontWeight: 500, color: c.slate, background: c.bg, borderTop: medTimer.length > 0 ? `1px solid ${c.line}` : 'none' }}>
